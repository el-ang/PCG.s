/**
 * PCG Random Number Generation for TS.
 * 
...
import { createInterface as cli } from 'readline';
...
const
     ...
     * @returns The new PCG RNG object
     */
    rng = () => new PCG(ds? 42: Date.now(), ds? 54: (Math.random() * (10 ** 19))),
    ...
    main = (rng: PCG) => {
        ...
        for (let i = 1, j: number, d: number[] = [], o = '', x = ''; i <= r; ++i) {
            o = 'Round ' + i + ':\n  64bit:';
            ...
            /* Make a long number */
            o += '\n  Combo: ';
            for (j = 0; j < 6; ++j) x += {0: rng.clip(-1n * (10n ** 19n), 10n ** 19n), 3: rng.clip(0, 9, !0)}[j] ?? rng.roll;
            console.log(o += ((x.length < 113)? x + rng.bind(10n ** BigInt(113 - x.length)): x.slice(0, 113)) + '\n');
        }
    }

/**
 * Command Line Interface (CLI) Menu
 * @param l - The command line input
 */
rl.on('line', (l: string) => {
    /**
     * Menu selection:
